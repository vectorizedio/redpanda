...
multi_snapshot_manager::open_snapshot(ss::sstring filename) {
...
multi_snapshot_manager::start_snapshot(ss::sstring target) {
...
ss::future<> multi_snapshot_manager::finish_snapshot(snapshot_writer& writer) {
    return ss::rename_file(writer.path().string(), writer.target().string())
      .then([this] { return ss::sync_directory(_dir.string()); });
}

ss::future<> multi_snapshot_manager::remove_partial_snapshots() {
...
ss::future<> multi_snapshot_manager::remove_snapshot(ss::sstring target) {
